...
const FullHeader = ({ sidebarOpen, setSidebarOpen }) => {
  // const handleClick = () => {
  //   console.log("rise in the world..", open);
  //   setOpen(true); // Call the function from props to update the state in the parent component
  // };
    ...
    <header className="sticky top-0 z-999 flex w-full bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none border-b-[1px] border-gray-300 drop-shadow-md">
      <div className="flex flex-grow items-center justify-between  py-2 px-2 shadow-2 md:px-6 2xl:px-11">
        <div className="flex items-center gap-2 sm:gap-4 lg:hidden">
          ...
          {/* <button onClick={handleOpenChange}>Open</button> */}
          <button
          ...
          >
            <span className="relative block h-5 w-5 cursor-pointer">
              <span className="block absolute right-0 h-full w-full bottom-1">
                <span
                  className={`relative top-0 left-0 my-1 block h-1 w-5 rounded-sm bg-primary delay-[0] duration-200 ease-in-out dark:bg-white ${!sidebarOpen && '!w-full delay-300'
                    }`}
                ></span>
                <span
                  className={`relative top-0 left-0 my-1 block h-1 w-5 rounded-sm bg-primary delay-150 duration-200 ease-in-out dark:bg-white ${!sidebarOpen && 'delay-400 !w-full'
                    }`}
                ></span>
                <span
                  className={`relative top-0 left-0 my-1 block h-1 w-5 rounded-sm bg-primary delay-200 duration-200 ease-in-out dark:bg-white ${!sidebarOpen && '!w-full delay-500'
                    }`}
                ></span>
              </span>
              <span className="absolute right-0 h-full w-full rotate-45">
                <span
                  className={`absolute left-2 top-0 block h-full w-1 rounded-sm bg-primary delay-300 duration-200 ease-in-out dark:bg-white ${!sidebarOpen && '!h-0 !delay-[0]'
                    }`}
                ></span>
                <span
                  className={`delay-400 absolute left-0 top-2 block h-1 w-full rounded-sm bg-primary duration-200 ease-in-out dark:bg-white ${!sidebarOpen && '!h-0 !delay-200'
                    }`}
                ></span>
              </span>
            </span>
